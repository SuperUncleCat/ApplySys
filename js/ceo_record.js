wrap_wait=$("#nav-ceo-wait");
wrap_success=$("#nav-ceo-allow");
wrap_fail=$("#nav-fail");
wrap_all=$("#nav-all");
wrap_search=$("#formS");
wrap_it=$('#nav-it-wait');
  ...
  $.ajax({
    ...
    success:function(data){
      ...
      $.each(json,function(index,response){
          if((response.user_class == '新入社員' || response.user_class == '異動' || response.user_class == '廃止' || response.user_class == '休止')&&response.ceo_is_good==2){
            num = response.id;
            ceo_good_check = response.ceo_is_good;
            ...
            if(response.depart_is_good == 2){
              depart_result = "<button type='button' class='btn btn-outline-warning btn-sm clearfix'>所属長承認待ち</button>";
            }else if(response.depart_is_good == 1){
              depart_result = "<button type='button' class='btn btn-outline-secondary btn-sm clearfix'>所属長承認済み</button>";
            }
            wrap_wait.append("<form action='show_details.php?id="+response.id+"' method='POST' target='_blank'><input type='hidden' name='id' value="+response.id+"><input type='hidden' name='ceo_good_check' value="+response.ceo_is_good+"><input type='hidden' name='apperant_check' value="+response.is_apperant+"><div class='card'><div class='card-body'><div class='card-text row'><div class='col-sm-3'>"+"<i class='far fa-address-card text-danger'></i>"+"使用者名:"+" "+response.user_name+"</div>"+"<div class='col-sm-3'>"+"部署:"+" "+response.user_department+"</div>"+"<div class='col-sm-3'>"+" "+hr_result+" "+depart_result+"</div>"+"<div class='col-sm-2'><button type='button' class='btn btn-outline-info btn-sm' onclick='isGood("+num+","+ceo_good_check+","+apperant_check+")';>許可</button><button type='button' class='btn btn-outline-info btn-sm btn-mp-1' data-toggle='modal' data-target='#reasonCenter' onclick='isBad("+num+","+ceo_good_check+")';>不許可</button>"+"</div>"+"<div class='col-sm-1 text-right'><button type='submit' class='btn btn-outline-info btn-sm'>詳細</button>"+"</div></div></div></div></form>")
          }
      })
    }
  })
  ...
  $.ajax({
    ...
    success:function(data){
      ...
      $.each(json,function(index,response){
        ...
        if((response.user_class == '新入社員' || response.user_class == '異動' || response.user_class == '廃止' || response.user_class == '休止')&&response.is_apperant==1&&response.ceo_is_good==1){
          num = response.id;
          wrap_success.append("<form action='show_details.php?id="+response.id+"' method='POST' target='_blank'><input type='hidden' name='id' value="+response.id+"><div class='card'><div class='card-body'><div class='card-text row'><div class='col-sm-3'>"+"<i class='far fa-address-card text-danger'></i>"+"使用者名:"+" "+response.user_name+"</div>"+"<div class='col-sm-3'>"+"部署:"+" "+response.user_department+"</div>"+"<div class='col-sm-5'>"+" "+depart_result+" "+hr_result +  " "+ ceo_result + " " + sys_result + "</div>"+"<div class='col-sm-1 text-right'><button type='submit' class='btn btn-outline-info btn-sm'>詳細</button></div></div></div></div></form>")
        }
      })
    }
  })
...
function notAllowedData(){
  ...
  $.ajax({
    ...
    success:function(data){
      json=JSON.parse(data);
      $.each(json,function(index,response){
        if(response.user_class=='新入社員'&&response.ceo_is_good==0){
          num = response.id;
...
function allData(){
  ...
  $.ajax({
    ...
    success:function(data){
      json=JSON.parse(data);
      $.each(json,function(index,response){
        if ((response.user_class == '新入社員' || response.user_class == '異動' || response.user_class == '廃止' || response.user_class == '休止') && response.ceo_is_good == 0) {
            num = response.id;
        ...
        }else if((response.user_class == '新入社員' || response.user_class == '異動' || response.user_class == '廃止' || response.user_class == '休止') && response.ceo_is_good == 1){
            num = response.id;
            wrap_all.append("<form action='show_details.php?id=" + response.id + "' method='POST' target='_blank'><input type='hidden' name='id' value=" + response.id + "><div class='card'><div class='card-body'><div class='card-text row'><div class='col-sm-3'>" + "使用者名:" + " " + response.user_name + "</div>" + "<div class='col-sm-3'>" + "部署:" + " " + response.user_department + "</div>" + "<div class='col-sm-3'>" + " " + "更新日:" + response.updated_at + "</div>" + "<div class='col-sm-1'>" + "<button type='button' class='btn btn-outline-success btn-sm text-right'>社長承認済み</button></div>" + "<div class='col-sm-2 text-right'><button type='submit' class='btn btn-outline-info btn-sm'>詳細</button></div></div></div></div></form>");
        }else if((response.user_class == '新入社員' || response.user_class == '異動' || response.user_class == '廃止' || response.user_class == '休止') && response.ceo_is_good == 2){
          num = response.id;
          wrap_all.append("<form action='show_details.php?id=" + response.id + "' method='POST' target='_blank'><input type='hidden' name='id' value=" + response.id + "><div class='card'><div class='card-body'><div class='card-text row'><div class='col-sm-3'>" + "使用者名:" + " " + response.user_name + "</div>" + "<div class='col-sm-3'>" + "部署:" + " " + response.user_department + "</div>" + "<div class='col-sm-3'>" + " " + "更新日:" + response.updated_at + "</div>" + "<div class='col-sm-1'>" + "<button type='button' class='btn btn-outline-primary btn-sm text-right'>社長承認待ち</button></div>" + "<div class='col-sm-2 text-right'><button type='submit' class='btn btn-outline-info btn-sm'>詳細</button></div></div></div></div></form>");
        }
      })
    }
  })
...
// 社長許可
function isGood(num,ceo_good_check,apperant_check){
  $.ajax({
    type : "POST",
    url : "ceo_allow.php",
    data : {
...
window.onload = function(){
  loadData();
  $("#nav-ceo-wait-tab").addClass("active");
  $("#nav-ceo-wait").addClass("show active");
}
